...
use App\Actions\Tenants\Exam\Attempts\GetExamQuestions;
use App\Actions\Tenants\Exam\Attempts\RecordExamAnswer;
use App\Actions\Tenants\Exam\Attempts\StartExamAttempt;
...
class StudentExamController extends Controller
{
    ...
    public function show(Request $request, string $id): JsonResponse
    {
        ...
        return ApiResponse::success(['exam' => $exam, 'last_attempt' => $lastAttempt], 'Exam details retrieved.');
    }
    ...
    public function getAttemptQuestions(Request $request, string $attemptId): JsonResponse
    {
    ...
    public function submit(Request $request, string $attemptId): JsonResponse
    {
        ...
        try {
            $attempt = $this->finalizeAttempt->execute($attempt, $request->user('tenant'));
        } catch (\RuntimeException $e) {
            return ApiResponse::error($e->getMessage(), 422);
        }

        return ApiResponse::success(['attempt_id' => $attempt->id], 'Exam submitted for grading.', 202);
    }
    ...
    public function timeRemaining(Request $request, string $attemptId): JsonResponse
    {
        ...
        return ApiResponse::success(['remaining_seconds' => $remaining, 'expired' => $remaining <= 0], 'Time remaining retrieved.');
    }
    ...
    public function logSuspiciousEvent(Request $request, string $attemptId): JsonResponse
    {
        ...
        $attempt->logSuspiciousEvent(SuspiciousEventType::from($validated['type']), $validated['metadata'] ?? []);
        $attempt->save();
    ...
    {
        $attempt = ExamAttempt::with(['exam.examQuestions', 'answers.question.options'])->findOrFail($attemptId);
        ...
        if (! $attempt->exam->isPublished()) {
            return ApiResponse::error('Results for this exam have not been released yet.', 403);
        }
    ...
    public function results(Request $request): JsonResponse
    {
        ...
        $attempts = ExamAttempt::with(['exam.subject', 'exam.examQuestions', 'answers.question.options'])
            ->where('student_id', $request->user('tenant')->id)
            ->whereIn('status', [ExamAttemptStatus::Graded, ExamAttemptStatus::Disqualified, ExamAttemptStatus::Timed_out])
            ->whereHas('exam', fn ($q) => $q->whereNotNull('published_at'))
            ->when(isset($validated['exam_id']), fn ($q) => $q->where('exam_id', $validated['exam_id']))
            ->latest('submitted_at')
        ...
        return ApiResponse::paginated($attempts, 'Results retrieved successfully.', $results);
    }
