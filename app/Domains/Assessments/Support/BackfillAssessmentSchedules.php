...
/**
 * Corrective backfill upgrading any pre-"global assessment" tenant database
 * to the occurrence model. Three generations are handled by one code path:
 *
 ...
 *
 * Every step checks what already exists first, so partially-migrated
 * databases — including tenants that recorded the earlier version of the
 * migration and can therefore never re-run it — are upgraded idempotently.
 * Shared by the corrective migration and the
 * `tenants:ensure-assessment-schedule-format` command.
 */
final class BackfillAssessmentSchedules
{
    /** True when this database still binds definitions to classes. */
    public function isLegacyFormat(): bool
    {
        return Schema::hasColumn('assessments', 'class_level_id');
    }
    ...
    private function moveClassBindingsToSchedules(): void
    {
        ...
        if (! $this->hasIndex('assessment_schedules_assessment_id_class_level_id_term_id_unique')) {
            Schema::table('assessment_schedules', function (Blueprint $table): void {
