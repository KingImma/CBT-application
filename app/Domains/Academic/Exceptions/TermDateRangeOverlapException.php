...
class TermDateRangeOverlapException extends BaseDomainException
{
    public function __construct(string $conflictingName, string $start, string $end, ?\Throwable $previous = null)
    {
        parent::__construct("The date range overlaps with an existing term ('{$conflictingName}' — {$start} to {$end}).", 422, $previous);
    }
}
